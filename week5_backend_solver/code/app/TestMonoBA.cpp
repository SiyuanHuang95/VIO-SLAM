...
void GetSimDataInWordFrame(vector<Frame> &cameraPoses, vector<Eigen::Vector3d> &points) {
    ...
    for (int j = 0; j < featureNums; ++j) {
        ...
        for (int i = 0; i < poseNums; ++i) {
            ...
            Pc = Pc / Pc.z();  // 归一化图像平面
            Pc[0] += noise_pdf(generator); // Add some noise
...
int main() {
    ...
    for (size_t i = 0; i < cameras.size(); ++i) {
        ...
        // 4 quaternion + 3 translation, but we only have 6 DOF, so we need to tell the solver the true DoF here
...
//        double inverse_depth = 1. / Pc.z();
    ...
    problem.Solve(5); // iterate for 5 steps
