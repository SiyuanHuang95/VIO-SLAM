...
public:
  ...
  virtual std::string TypeInfo() const {
    return "abc";
  } // TypeInfo is only used for the reminder to the programmer, Debug useage
};
...
public:
  ...
  //    residual_(0) =                          // residual has only one
  //    dimension
  //        std::exp(abc(0) * x_ * x_ + abc(1) * x_ + abc(2)) - y_; // 构建残差
  //  }

  // homework for ax^2 + bx + c
  virtual void ComputeResidual() override {
    Vec3 abc = verticies_[0]->Parameters(); // 估计的参数
    residual_(0) =                          // residual has only one dimension
        abc(0) * x_ * x_ + abc(1) * x_ + abc(2) - y_; // 构建残差
  }
  // 计算残差对变量的雅克比
  // Jacobian is w.r.t to the parameter, which is aimed for optimization
...
int main() {
  double a = 20.0, b = 20.0, c = 10.0; // 真实参数值
  int N = 100;                         // 数据点
  ...
  // 构建 problem
  // here we set to be GENERIC_PROBLEM
